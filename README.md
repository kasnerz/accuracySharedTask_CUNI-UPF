...
Use the script `generate.py` for preprocessing the annotated data from the shared task.
...
For both annotated and synthetic data, the versions of the dataset using context sizes 5, 10, 20 and 40 are generated automatically. If you wish to specify a custom context size, use the parameter `--ctx`. The context size is denoted in the folder name by suffix `_ctx{CONTEXT_SIZE}`.

### Training
Context size 40 is used in all the examples.
#### Annotated data
For training the model on the **annotated** data, use the following command:
```bash
./train.py \
...
#### Annotated + synthetic data
For training the model on both the **synthetic** and **annotated** data:
1. Train the model on the synthetic data (see the previous section).
2. Load the trained checkpoint and further train it on annotated data:
```bash
./train.py \
    --dataset ann_compact_ctx40 \
